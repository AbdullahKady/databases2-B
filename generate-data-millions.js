...
let temp_player, temp_name,
  playersInCurrentMatch; //To ensure primary key (no 2 players with the same name in a match)

for (let counter = 0; counter < 38; counter++) {
  let playersChunk = ``;
  let matchesChunk = ``;
  //Chunk it to smaller blocks (125k records each) to avoid heap overflow
  for (let i = counter * 125000 + 1; i <= counter * 125000 + 125000; i++) {
  ...
  console.log(`DONE ${counter + 1}`);
}

//PELES (250k)
for (let counter = 38; counter < 40; counter++) {
  let playersChunk = ``;
  let matchesChunk = ``;
